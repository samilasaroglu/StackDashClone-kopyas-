...
public class MobileInput : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!RaycastSystem.instance.move)
        {
            ...
            if (swipeDelta.magnitude > deadZone)
            {
                ...
                if (Mathf.Abs(x) > Mathf.Abs(y))
                {
                    ...
                    if (x < 0)
                    {
                        // sol
                        swipeLeft = true;
                        karakter.transform.eulerAngles = -Vector3.up * 90;
                    }
                    else
                    {
                        // sağ
                ...
                else
                {
                    ...
                    if (y < 0)
                    {
                        // aşağı
                        swipeDown = true;
                        karakter.transform.eulerAngles = Vector3.up * 180;
                    }
                    else
                    {
                        // yukarı
                        swipeUp = true;
                        karakter.transform.eulerAngles = Vector3.zero;
                    }
                }
